...
![CourseFrame](docs/screenshot.png)
...
| **Manual mode** | Build block by block, no API key needed |
| **11 block types** | Text, Callout, Image, Accordion, Tabs, Quiz, Process, Timeline, Sorting, Button, Divider |
| **Inline editing** | Click any text in the preview to edit it |
...
```bash
git clone https://github.com/linuxsunil/courseframe.git
cd courseframe
...
| **Copilot** | [azure.microsoft.com](https://azure.microsoft.com/products/ai-services/openai-service) |

**No API key?** Use Manual mode — build every block yourself with no AI required. Or use the [manual prompt](docs/manual-prompt.txt) to generate content in any AI chat tool and paste the result back in.
...
```
1. Setup        Set your course title, lessons, brand colour, and AI provider
2. Build        Paste raw SME notes per lesson → Generate blocks with AI
                (or add blocks manually — your choice)
3. Preview      Click through the full Rise-style prototype
                Edit any text inline — it's all live
4. Export       Download a single HTML file
5. Share        Email it, upload to Drive, send on WhatsApp
...
- Works offline, on a plane, in a client meeting
...
- [ ] Drag-to-reorder blocks in preview
- [ ] Multiple export themes
- [ ] SCORM metadata header option
- [ ] Notion / Google Docs import
- [ ] Shareable link via GitHub Gist

Have an idea? [Open an issue](https://github.com/linuxsunil/courseframe/issues) or see [CONTRIBUTING.md](CONTRIBUTING.md).
